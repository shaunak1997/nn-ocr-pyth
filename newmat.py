...
img = sc.loadmat('ex4data1.mat')
mat = img['X']
mat1 = mat[3608]
mat1 = mat1.reshape((20,20))
mat1 = mat1.transpose()
new1 = cv2.resize(mat1,(200,200),interpolation=cv2.INTER_AREA)
cv2.imshow('hm',new1)
for i in range(0,20):
	for j in range(0,20):
		if(mat[i,j] < 0):
...
for i in range(0,20):
	for j in range(0,20):
		if(mat1[i,j] > 0.5):
			...
			mat1[i,j] = 0
img = mat1.astype(np.uint8)

#zhang_suen = cv_algorithms.zhang_suen(img)
zhang_suen = cv_algorithms.guo_hall(img)
#cv2.imshow('thin',zhang_suen)

img1 = zhang_suen.astype(np.float64)
for i in range(0,20):
	for j in range(0,20):
		img1[i,j] = (img1[i,j]/255)
new = cv2.resize(mat1,(40,40),interpolation=cv2.INTER_AREA)
cv2.imshow('thin',new)
output = np.zeros((10))
output[0] = 1
